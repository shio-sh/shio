...
// IO write callback for external IO backend. Invoked when the terminal
// wants to send bytes back to the embedder (user input, OSC responses,
// mouse events, etc). May be called from a non-main thread.
typedef void (*ghostty_io_write_cb)(void* userdata,
                                    const uint8_t* ptr,
                                    size_t len);

// IO resize callback for external IO backend. Invoked when the terminal
// grid is resized; the embedder typically forwards this to a remote PTY
// via SIGWINCH (over SSH window-change request, etc).
typedef void (*ghostty_io_resize_cb)(void* userdata,
                                     uint16_t columns,
                                     uint16_t rows,
                                     uint32_t width_px,
                                     uint32_t height_px);
...
typedef struct {
  ...
  // Selects the IO backend. DEFAULT spawns a shell via fork/exec (the
  // historical behavior). EXTERNAL disables fork/exec entirely and uses
  // the io_write_callback / ghostty_surface_write_bytes path instead.
  uint8_t io_backend;
  ghostty_io_write_cb io_write_callback;
  ghostty_io_resize_cb io_resize_callback;
  void* io_userdata;
...
GHOSTTY_API void ghostty_config_load_string(ghostty_config_t,
                                            const char*,
                                            size_t);
...
// Push bytes into the terminal as if they came from a remote pty. Only
// meaningful when the surface was created with the EXTERNAL io_backend.
// Safe to call from any thread; takes the renderer state lock internally.
GHOSTTY_API void ghostty_surface_write_bytes(ghostty_surface_t,
                                             const uint8_t*,
                                             uintptr_t);
