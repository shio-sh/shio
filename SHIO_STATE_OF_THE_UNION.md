...
## 8. In‑flight: away‑push debugging (IMPORTANT — pick up here)

The lock‑screen approve loop is built and the Mac side is verified, but **the iPhone wasn't receiving the CloudKit push**. Root cause found via logging:

```
[shio] push: registerIfAuthorized authStatus=0 (notDetermined)
[shio] push: NOT authorized — skipping registerForRemoteNotifications
```

The app had **never actually requested notification permission** — the request (`PushService.requestAuthorizationAndRegister()`, which prompts → registers → gets the APNs token) was wired only to *starting a session*, and the tester went straight to Settings → "Send test notification." So: `notDetermined` → no `registerForRemoteNotifications` → no APNs token → CloudKit had nowhere to deliver, even though the `Signal` write and the `shio-signal-subscription` both succeed.

**Fix applied (latest commit):** the test button now calls `requestAuthorizationAndRegister()` (prompts, then registers). **Next step to verify on device:** tap "Send test notification" → Allow the prompt → tap again → it should report `Push token: …xxxx ✓` → the banner (with Approve/Deny) arrives. Then run the full y/n loop.

**Likely follow‑up:** the *permission request* should happen at a more sensible, discoverable moment than "first session start" — consider requesting it when the user first enables away‑push / opens a project, with a clear pre‑prompt. There are temporary `print("[shio] push: …")` diagnostics in `PushService` to remove once it's confirmed working.
...
xcodebuild -scheme Shio   -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 16' build
```
