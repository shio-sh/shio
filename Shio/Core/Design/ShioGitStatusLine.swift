...
/// The git line — "⎇ branch ↑2 ↓1 ·3" (or clean) plus an optional PR chip —
/// rendered identically wherever a repo shows its state. Staleness comes in
/// on the model: a stale cache reads "·· branch" at lowered opacity, never
/// as live numbers.
///
/// `compact` is the trailing-cluster form (dirty count + PR only) for rows
/// whose leading text already carries the branch.
struct ShioGitStatusLine: View {
    let model: GitLineModel
    var openPR: PullRequest? = nil
    ...
    var body: some View {
        HStack(spacing: compact ? 8 : 6) {
            ...
            if model.hasTracking {
                ...
                } else if !compact || openPR == nil {
                    Text(cleanMark).foregroundStyle(ShioTheme.success)
                }
            }
            if let openPR {
                Text("PR #\(openPR.number)")
                    .foregroundStyle(openPR.isDraft ? ShioTheme.textTertiary : ShioTheme.info)
            }
        }
