...
final class GhosttyMacSurface: NSView {
    ...
    init(backend: Backend = .local, frame frameRect: NSRect = .zero) {
        self.backend = backend
        super.init(frame: frameRect.isEmpty ? NSRect(x: 0, y: 0, width: 800, height: 500) : frameRect)
        wantsLayer = true
        commonInit()
    }
    required init?(coder: NSCoder) { self.backend = .local; super.init(coder: coder); wantsLayer = true; commonInit() }
    ...
    private func commonInit() {
        ...
        switch backend {
        case .local:
            // libghostty forks/execs the login shell and owns the PTY.
            // (Invisible-tmux wrapping for continuity lands with M3.)
